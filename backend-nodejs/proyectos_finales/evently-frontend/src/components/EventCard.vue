  ...
  <article class="rounded-2xl bg-white shadow-soft p-4 flex flex-col gap-2">
    <header class="flex items-center justify-between">
      ...
      <span class="text-xs px-2 py-0.5 rounded-full"
            :class="event.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'">
        {{ event.status }}
      </span>
    </header>
    ...
    <footer class="mt-2">
      <router-link :to="{ name:'event-detail', params:{ id: event.id } }"
                   class="px-3 py-1 rounded-xl bg-gray-900 text-white text-sm">Ver</router-link>
      <slot name="actions" />
    </footer>
  </article>
</template>
