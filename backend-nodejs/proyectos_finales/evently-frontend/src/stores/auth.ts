...
export const useAuthStore = defineStore('auth', () => {
  ...
  function _connectSocket() {
    ...
    socket.value.on('event-updated', (p) =>
      document.dispatchEvent(new CustomEvent('toast', { detail: { type: 'info', text: `Evento actualizado (${p.eventId})` } }))
    );
    socket.value.on('event-cancelled', (p) =>
      document.dispatchEvent(new CustomEvent('toast', { detail: { type: 'warn', text: `Evento cancelado (${p.eventId})` } }))
    );
  }
  ...
  return {
    user, accessToken, refreshToken, socket,
    isAuthenticated, role,
    loadFromStorage, register, login, refreshTokens, logout,
  };
});
